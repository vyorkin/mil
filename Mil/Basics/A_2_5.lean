...
-- (PartialOrder) и решётка (Lattice), где для любых двух элементов
-- определены инфимум x ⊓ y и супремум x ⊔ y как точная нижняя и
-- верхняя грань. Задача та же, что и в 2.2, — вывести стандартные
-- свойства (идемпотентность, законы поглощения, дистрибутивность там,
-- где она есть) из небольшого числа аксиом решётки.
--
-- Показывается связь между «≤» и «⊓»/«⊔»: x ≤ y равносильно x ⊓ y = x
-- (и двойственно для ⊔) — эта эквивалентность и характеризующие
-- свойства инфимума и супремума (например, le_inf) оказываются
-- рабочим инструментом почти во всех доказательствах раздела, а не
-- разбор «что чему равно поэлементно».
--
...
-- явно указанное b), что бывает нужнее, чем le_trans, когда среднее
-- звено неочевидно из контекста.
--
-- Далее аксиоматика усложняется: строго упорядоченное кольцо, где
-- порядок согласован с кольцевыми операциями (a ≤ b → a + c ≤ b + c,
-- а для умножения — только при неотрицательности множителя), и
-- метрическое пространство с его тремя аксиомами: неотрицательность
-- и нулевое расстояние до себя, симметричность, неравенство
-- треугольника.
...
--    В задаче выше я применил le_antisymm по инерции, потому что в
--    большинстве предыдущих доказательств нужно было использовать антисимметрию.
