...
-- Содержание раздела:
--
-- Здесь кольцо (Ring), группа (Group) и им подобные структуры
-- вводятся не как встроенные типы, а как аксиоматика: набор операций
-- (+, ×, обратный элемент) и тождеств, которым они подчиняются.
-- Доказательства ведутся не про конкретное R = ℝ или ℤ, а про
-- произвольную структуру, удовлетворяющую этим аксиомам, — то есть
-- сразу для всех колец/групп разом.
--
-- Практическая техника — как выводить неочевидные следствия
-- (например, a * 0 = 0) из небольшого набора аксиом, используя have
-- для промежуточных утверждений и apply/exact для точного попадания
-- в цель. Показывается, что многие такие следствия не входят в сами
-- аксиомы явно и их приходится конструировать вручную — это хорошая
-- тренировка «видеть» алгебраическую структуру доказательства.
--
-- Далее вводятся тактики-автоматизаторы для типовых алгебраических
-- тождеств: ring — для (коммутативных) колец, abel — для абелевых
-- групп в аддитивной записи, group — для произвольных групп в
-- мультипликативной записи, noncomm_ring — для некоммутативных колец.
-- Выбор тактики зависит от того, какой именно аксиоматике подчиняется
-- структура в конкретном доказательстве.
--
-- Отдельно обсуждается разница между аддитивной (+, 0, -a) и
-- мультипликативной (*, 1, a⁻¹) записью одной и той же по сути
-- групповой структуры — в Mathlib это два параллельных набора имён
-- лемм (AddGroup/Group, add_assoc/mul_assoc и так далее).
...
--    ∃ 0 ∈ R, a = a + 0 - сущ. нейтральный элемент для сложения
...
-- Когда линь его видит, он делает ровно то же самое, что и
...
-- Удобно пользоваться аддитивной нотацией, когда групповая операция коммутативна.
-- Когда коммутативности нет, то удобнее пользоваться мультипликативной нотацией.
...
-- Вспомогательная лемма, которая используется ниже.
...
-- сдвинув скобки влево ассоциативностью поменять a * a⁻¹ на 1,
...
-- выражение (a * b)⁻¹ в правую часть, а всё, что появится вследствие этого
