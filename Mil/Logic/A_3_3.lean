...
-- Содержание раздела:
--
-- Отрицание ¬P в Lean устроено как обычная импликация P → False,
-- поэтому все приёмы работы с импликацией (intro, apply) применимы
-- и здесь: чтобы доказать ¬P, нужно предположить P и вывести
-- противоречие; чтобы воспользоваться гипотезой ¬P, достаточно
-- применить её к доказательству P и получить False.
--
-- На примере ограниченности функции сверху/снизу (FnHasUb/FnHasLb)
-- показывается типичная задача — доказать, что некоторое свойство не
-- выполняется, и то, что естественная формулировка «не существует
-- границы» гораздо удобнее в работе после того, как отрицание
-- протолкнуто внутрь кванторов тактикой push_neg (¬∃ превращается в
-- ∀¬, ¬∀ — в ∃¬, а неравенства меняются на противоположные).
--
-- Вводятся by_contra (доказательство от противного: допускаем
-- отрицание цели и ищем противоречие), exfalso (явно заменить любую
-- цель на False, если уже есть из чего вывести противоречие) и
-- contradiction (автоматический поиск явно противоречащих друг другу
-- гипотез в контексте).
--
-- Отдельно обсуждается контрапозиция — переход от «P → Q» к
-- «¬Q → ¬P» тактиками contrapose/contrapose! (вторая версия сразу же
-- прогоняет push_neg по результату), — и то, что использование
-- исключённого третьего (P ∨ ¬P) и связанных с ним рассуждений
-- опирается на классическую, а не только конструктивную логику.
...
-- Покажем, что из lt_asymm следует lt_irrefl .
...
-- 3. Плохо понимаю как в целом доказывать противоречивость для неравенств.
...
--
-- contrapose! делает то же самое, что contrapose,
