...
-- Содержание раздела:
--
-- Симметричный переход от «для всех» к «существует». Чтобы доказать
-- «∃ x, P x», нужно явно предъявить конкретный x — свидетеля; для
-- этого служит тактика use, после которой остаётся доказать P уже
-- для этого конкретного x.
--
-- Обратная задача — использовать имеющуюся гипотезу вида
-- «∃ x, P x»: её нужно распаковать, получив в контекст конкретное
-- имя x и доказательство P x. Показывается несколько способов это
-- сделать: rcases/rintro с образцом ⟨x, hx⟩ (в том числе вложенным,
-- если P x сама содержит конъюнкцию или ещё один квантор), а также
-- obtain как более декларативный синоним и cases/match как более
-- общая конструкция для разбора данных.
--
-- На примерах показывается, что многие привычные понятия скрыто
-- экзистенциальны: «a делит b» — это «∃ c, b = a * c», а
-- сюръективность функции — это «∀ y, ∃ x, f x = y», то есть сочетание
-- обоих кванторов сразу. Разбирается композиция функций и то, как
-- доказательство сюръективности композиции собирается из
-- сюръективности каждой функции по отдельности — с использованием и
-- use, и rcases в одном доказательстве.
