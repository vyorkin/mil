...
-- 3.1. Импликация и квантор всеобщности.

-- Содержание раздела:
--
-- Открывает главу про логику и показывает ключевое наблюдение: в
-- Lean утверждение «∀ x, P x» и утверждение «P → Q» по сути устроены
-- одинаково — оба доказываются через intro (введи x или введи
-- допущение) и используются через применение к аргументу. Поэтому
-- лемма вида «∀ x y, x ≤ y → f x ≤ f y» (монотонность) доказывается
-- одним и тем же приёмом: intro x, intro y, intro h.
--
-- На примерах монотонности и чётности/нечётности функций
-- показывается, как разворачивается определение, спрятанное за
-- обозначением (например, Monotone f — это и есть тот самый
-- ∀-конструкт, просто под именем). Тактики dsimp и change служат
-- именно для этого: dsimp разворачивает определение по вычислению,
-- change позволяет явно подставить любую определённо равную
-- формулировку цели.
--
-- Обсуждаются неявные аргументы (в фигурных скобках) — Lean выводит
-- их сам из контекста, и явно писать их не нужно, — а также то, что
-- квантор всеобщности часто спрятан внутри самого определения
-- (например, в определении подмножества s ⊆ t). Дальше это
-- применяется к отношению подмножества и к инъективности функции —
-- оба определения по сути являются вложенными «для всех, если-то».
...
-- Когда научишься замечать кванторы всеобщности – ты будешь находить их везде.
