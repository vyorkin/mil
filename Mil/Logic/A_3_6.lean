...
-- Содержание раздела:
--
-- Завершает главу про логику первым по-настоящему «аналитическим»
-- сюжетом: формальное ε-N определение сходимости последовательности
-- (∀ ε > 0, ∃ N, ∀ n ≥ N, |s n - a| < ε) и перевод типичных «бумажных»
-- доказательств (сходится сумма/произведение, предел единственен,
-- сходящаяся последовательность ограничена) в Lean. Это первое место
-- в книге, где приходится по-настоящему комбинировать всю логическую
-- технику предыдущих пяти разделов в одном доказательстве: кванторы,
-- экзистенциальные свидетели (обычно — max двух N), импликации,
-- конъюнкции.
--
-- Типичная трудность, которую разбирает раздел, — на бумаге принято
-- сразу брать N = max N₁ N₂ «для обоих условий сразу», а в Lean этот
-- шаг приходится проговаривать явно, вместе с обоснованием, что
-- max N₁ N₂ ≥ N₁ и ≥ N₂.
--
-- Вводятся ext (доказательство равенства функций через равенство их
-- значений в каждой точке), congr (свести цель вида f a = f b к цели
-- a = b) и convert (аналог exact, применяемый, когда цель почти, но
-- не буквально совпадает с заключением леммы, — оставляет доказать
-- расхождения отдельно), а также simp как тактика упрощения по
-- набору лемм-тождеств.
--
-- В конце раздел обобщает всё это с ℝ на произвольное линейно
-- упорядоченное поле, показывая, что само доказательство не
-- использовало ничего специфичного для вещественных чисел, кроме
-- порядка и арифметики.
