...
-- Содержание раздела:
--
-- Дизъюнкция P ∨ Q: чтобы доказать P ∨ Q, нужно выбрать, какую из
-- двух половин доказывать, — тактики left/right (или, на уровне
-- термов, конструкторы Or.inl/Or.inr). Чтобы воспользоваться
-- гипотезой P ∨ Q, нужно рассмотреть оба случая отдельно — это
-- делает rcases с образцом «h1 | h2» (по одному имени на каждую
-- ветку).
--
-- На примере доказательства неравенства треугольника
-- |a + b| ≤ |a| + |b| через явный разбор знака a и b (abs_le,
-- le_or_gt и тому подобные) показывается стандартный сценарий:
-- чтобы доказать неравенство с модулем, распишем по случаям, где
-- выражение внутри модуля неотрицательно, а где отрицательно.
--
-- Вводится by_cases h : P — способ явно начать разбор случаев по
-- истинности любого (разрешимого) утверждения P, не обязательно уже
-- присутствующего в контексте как дизъюнкция, что по сути использует
-- закон исключённого третьего как отправную точку доказательства.
--
-- Далее это применяется к представлению чисел в виде суммы квадратов
-- и к отсутствию делителей нуля в области целостности (если
-- a * b = 0, то a = 0 ∨ b = 0) — оба сюжета требуют именно разбора
-- случаев, а не прямого вычисления.
...
-- В этой книге мы обычно будем использовать rcases для дизъюнкций.
...
-- Тактика by_cases h : P делает то же самое, что и cases em P.
-- Только в отличие от cases em P, она позволяет дать название
