...
-- Содержание раздела:
--
-- Конъюнкция P ∧ Q рассматривается как пара доказательств: чтобы
-- доказать P ∧ Q, нужно доказать обе части по отдельности (тактика
-- constructor разбивает цель на две подцели), а если P ∧ Q уже есть
-- в контексте, из неё можно вытащить любую из двух частей через
-- .1/.left и .2/.right (или сразу распаковать через rcases/obtain,
-- как в 3.2).
--
-- Эквивалентность P ↔ Q трактуется параллельно: как по сути пара
-- импликаций в обе стороны, доказываемая тем же constructor, и как
-- значение, у которого есть направления .mp (P → Q) и .mpr (Q → P).
-- Отдельно показывается, что ↔-утверждение можно использовать в rw
-- так же, как равенство, — переписывая одну сторону эквивалентности
-- на другую прямо внутри цели.
--
-- Материал закрепляется на характеризации чётности/нечётности через
-- ↔ и на связи между предпорядком, частичным порядком (это ≤ плюс
-- дополнительно антисимметричность) и строгим порядком <, который
-- определяется как a ≤ b ∧ a ≠ b, — то есть строгий порядок буквально
-- есть конъюнкция, что даёт естественную практику для всех введённых
-- в разделе тактик разом.
...
example (x y : ℝ) : (x ^ 2 + y ^ 2 = 0) ↔ (x = 0 ∧ y = 0) := by
  ...
  --   rcases h with ⟨h₀, h₁⟩
  --   rw [h₀, h₁]
  -- ^ Эти 3 строчки выше эквивалентны вот этой одной:
  · rintro ⟨rfl, rfl⟩
    norm_num
...
example : ¬Monotone fun x : ℝ ↦ -x := by
  ...
  -- она была тебе дана сразу выше. Но моё полностью эквивалентно.
  use 0
  use 1
  norm_num
