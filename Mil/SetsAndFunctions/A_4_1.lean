...
-- Содержание раздела:
--
-- Открывает главу про множества и функции с фундаментального для
-- Lean/Mathlib устройства: множество Set α — это не отдельная
-- сущность, а предикат α → Prop, а «x ∈ s» по определению
-- равносильно «s x». Отсюда следует, что почти все доказательства
-- про множества на самом деле являются доказательствами про
-- предикаты и в них работают те же логические тактики из глав 2–3.
--
-- Показывается, как через это разворачиваются стандартные операции:
-- s ∩ t, s ∪ t, s \ t, s ⊆ t — каждая определена через ∧, ∨, ¬ и →
-- на уровне предикатов, а лемма вида mem_inter_iff — это просто имя
-- для соответствующего разворачивания. Основной инструмент для
-- доказательства равенства множеств — тактика ext: она сводит s = t
-- к «∀ x, x ∈ s ↔ x ∈ t», после чего в дело идёт вся техника работы
-- с ↔ из 3.4. Альтернативный путь — доказать s ⊆ t и t ⊆ s отдельно
-- и собрать через Subset.antisymm (антисимметрия ⊆).
--
-- Вводятся ограниченные кванторы ∀ x ∈ s, ... и ∃ x ∈ s, ... как
-- сокращения для ∀ x, x ∈ s → ... и ∃ x, x ∈ s ∧ ..., и их
-- индексированные варианты: ⋃ i, A i / ⋂ i, A i (объединение и
-- пересечение семейства множеств, занумерованного индексом i) и
-- ⋃₀ s / ⋂₀ s (объединение и пересечение множества множеств, то есть
-- s : Set (Set α)) — с соответствующими характеризующими леммами
-- mem_iUnion, mem_iInter, mem_iUnion₂ и так далее для случая с двумя
-- индексами сразу.
...
example : s \ (t ∪ u) ⊆ (s \ t) \ u := by
  ...
  constructor
  · use xs
    -- Нет, ты не можешь делать rcases на функции,
...
--    сделать apply hnot, где hnot - какая-то гипотеза с отрицанием.
